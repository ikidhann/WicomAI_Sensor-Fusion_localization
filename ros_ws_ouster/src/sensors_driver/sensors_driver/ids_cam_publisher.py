#### Run ros2 run sensors_driver cam_publisher_node --ros-args -p camera_name:=ids
import rclpy
...
class CameraPublisher(Node):
    def __init__(self):
        super().__init__('ids_cam_publisher_node')

        # ------------------------- PARAMETERS -------------------------
        self.declare_parameter('camera_name', 'ids')   # <--- NEW
        ...
        # ------------------------- PUBLISHERS -------------------------
        self.raw_img_pub_ = self.create_publisher(Image, '/ids/image', 10)
        self.cam_info_pub_ = self.create_publisher(CameraInfo, '/ids/camera_info', 10)
