...
class DroneLocalization(Node):
    def __init__(self):
        ...
        # add distance buffer
        self.distance_buffer_ = deque(maxlen=50)
        self.get_logger().info("Drone Localization Node started!")
        

    def _pc_projection(self, xyz_transformed, P, img_h, img_w):

        pc_3d = xyz_transformed.T
        ...
        try:
            ...
            for point in projected_pcd:
                ...
                if masks[v, u] > threshold:
                    ...
                    #print(f"Drone mask point: u={u}, v={v},z_cam={z_cam:.2f}m")
                    #self.get_logger().info(f"u={u}, v={v}, z_cam value:{z_cam:.2f}m")
            ...
            if len(distances) != 0:
                ...
                unfilter_distance = np.asarray(distances, dtype= float)
                print(unfilter_distance.dtype)
                valid_distance = unfilter_distance[np.isfinite(unfilter_distance)& (unfilter_distance <23)]
                if valid_distance.size>0:
                    relative_distance = float(np.mean(valid_distance))

                self.distance_buffer_.append(relative_distance)

                avg_distance = float(np.max(self.distance_buffer_))
                ...
                #cv2.putText(cv_image, f'Localization Distance : {relative_distance:.2f} m', (10, 40),
                #cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)

                log_msg = f'Localized Drone Distance: {avg_distance:.2f} m'
                cv2.putText(cv_image, f'Distance : {avg_distance:.2f} m', (10, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)
            else:
    ...
    def localization_callback(self, pcd_msg, raw_image_msg, cam_info_msg, drone_mask_msg):
        ...
        # Extract XYZ-only and apply TF manually (avoids dtype mismatch with Ouster fields)
        ...
        xyz_transformed = (R @ xyz.T).T + np.array([t.x, t.y, t.z])
        ...
        projected_pcd, proj_info = self._pc_projection(xyz_transformed, P, IMG_H, IMG_W)
