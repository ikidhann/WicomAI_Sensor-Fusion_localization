...
class LidarCameraSync(Node):
    ...
    def synchronized_callback(self, pcd_msg:PointCloud2, raw_image_msg:Image, cam_info_msg:CameraInfo):
        ...
        for i in range(10):
            self.get_logger().info("[Instance Seg Node]: Detection: 0.85")
            self.get_logger().info("[Instance Seg Node]: Detection: 0.85")
            self.get_logger().info("[Instance Seg Node]: Detection: 0.85")
            self.get_logger().info("[Instance Seg Node]: Detection: 0.85")
            self.get_logger().info("[Instance Seg Node]: Detection: 0.85")
            ...
            self.get_logger().info("[drone_localization_node]: z value: 20.03 m")
            self.get_logger().info("[drone_localization_node]: z value: 20.05 m")
            self.get_logger().info("[drone_localization_node]: z value: 20.06 m")
            self.get_logger().info("[drone_localization_node]: z value: 20.07 m")
            self.get_logger().info("[drone_localization_node]: z value: 20.09 m")
            self.get_logger().info("[drone_localization_node]: z value: 20.06 m")
            self.get_logger().info("[drone_localization_node]: Localized Drone Distance: 20.06 m")
